    ...
    while(ros::ok()
        ...
        switch (status){
            ...
            case 3://展開動作
                ...
                if(spreaded == 0 && wait_num == 0 && (towel[0] || towel[1])){
                    now_t = time(nullptr);
                    fprintf(fp,"展開 now(%f,%f,%f)\tspread:%d,wait_num%d%ld\n",now_x,now_y,now_yaw,spreaded,wait_num,now_t - start_t);
                    sendMechanism(5);//第一展開
                    status = 3;
                    next_move = 3;
                }else if((wait_num == 0 || (!towel[0] && !towel[1] && seats)) && fabs(now_x) > 1500){
                    status = 4;
                    next_move = 4;
                }
                break;
            ...
            case 5://バスタオルを干す位置に移動する
                ...
                if(first_move){
                    ...
                    first_move = false;
                }else if((bath == 0 && !towel_switch) || (bath == 1 && towel_switch)){
            ...
            case 6://移動補正
                ...
                if(count_towel > (50 - (fight || !bath)*30)){
                    skip = true;
                    ROS_INFO("towel timeout");
                    ...
                    break;
                }else if(towel_x_ok && fabs(now_x - goal_x) < 40 && fabs(now_y - goal_y) < 25 && fabs(now_v_y) < 40 && fabs(now_yaw) < 2){
                    count_towel++;
                ...
                if(!towel_y_ok && fabs(goal_x - now_x) < 600 && fabs(goal_y - now_y) < 300 && bath == 0 && (!sheet_first || !fight )){
                    towel_y_ok = true;
                    auto_move = true;
                }
                if(fabs(now_x - point[num+4][coat][0]) < 800 && !towel_y_ok && bath == 1 && (!sheet_first || !fight)){
                    //next_move = 7;
                    towel_y_ok = true;
                    goal_y = TOWEL_Y + 100;
                    auto_move = true;
                    ...
                    if(fabs(lidar_x_diff) > 400 || fabs(now_x - point[num+4][coat][0]) > 400/* || (coat ? fabs(now_x + lidar_x + 100) : fabs(now_x + lidar_x - 4830)) < 300*/){
                        lidar_x_diff = lidar_x - offset[num][coat][3];//台座
                        if(fabs(lidar_x_diff) < 50 && fabs(now_x - point[num+4][coat][0]) < 150){//振動防止
                ...
                if(!lidar_x_ok){
                    if(fabs(now_x - goal_x) < 400 && fabs(tmp_goal_x - point[num+4][coat][0]) < 250 && rs_data_x != 0){
                        if(rs_data){
                            goal_x = tmp_goal_x;
                ...
                if(towel_y_ok && (count_hang >= 15 || bath == 0) && fabs(now_x - goal_x) < (10 - (bath && !fight)*3) && fabs(now_v_x) < (30 - (bath && !fight)*10) && fabs(now_yaw) < 1 && !towel_x_ok){
                    towel_x_ok = true;
                    next_move = 7;
                    auto_move = true;
                    goal_y = TOWEL_Y + 30;
                    about_move = true;
                    if(rs_data_x != 0 && fabs(rsx_diff) < 200){
                        rs_goal_x = rs_data_x;
                    }
                    ...
                    }else if(tmp_goal_y > (TOWEL_Y - 200) && tmp_goal_y < (TOWEL_Y + 200) && /*rsy_diff > -150*/ fabs(now_y - goal_y) < 200 && (now_y - TOWEL_Y) < 200){
                        if(rs_data){
                            goal_y = tmp_goal_y;
            ...
            case 11://補正動作
                ...
                if(limit_left){
                    if(((fabs(rsy_diff) < 7 && !lidar_x_ok) || (fabs(lidar_y_diff) < 7 && lidar_x_ok) || sheet_x_ok) && fabs(goal_y - now_y) < 10 && fabs(now_v_y) < 30 && spreaded == 2){
                        status = 12;
                        next_move = 12;
                        skip = true;
                ...
                }else if((now_x - tmp_goal_x < 20 || lidar_x_diff > -20) && fabs(goal_x - now_x) < 50 && next_move == 12 && fabs(now_y - goal_y) < 70){
                    goal_x = now_x - 50;
                    fprintf(fp,"シーツリミットまで RS_X:%d,%d\tG(%d,%d),N(%d,%d,%f)\tlidar(%d,%d)\n",(int)rs_data_x,(int)rsx_diff,(int)goal_x,(int)goal_y,(int)now_x,(int)now_y,now_yaw,lidar_x,lidar_y);
                }else if(fabs(rsx_diff) < 600 && fabs(tmp_goal_x - point[2][coat][0]) < 300){
                    //goal_x = now_x - 50;
                    if(rs_data){
                        goal_x = tmp_goal_x;
                ...
                if(spreaded != 2 && wait_num == 0 && fabs(now_y - goal_y) < 50 && fabs(now_x - goal_x) < 50){
                    ...
                    }else if(fabs(lidar_y_diff) < 400 && goal_y < (SEATS_Y + 200) && fabs(now_yaw) < 1){
                        goal_y = now_y + lidar_y_diff;
                        lidar_x_ok = true;
                        sheet_x_ok = false;
                    ...
                    }else if(fabs(SEATS_Y  - tmp_goal_y) < 300 && tmp_goal_y < (SEATS_Y + 150)&& rsy_diff < 120 && (now_y - SEATS_Y) < 200 && rs_data_y != 0){
                        if(rs_data){
                            goal_y = tmp_goal_y;
                            lidar_x_ok = false;
                            sheet_x_ok = false;
            ...
            case 14://補正動作
                ...
                if(limit_right){
                    ...
                    lidar_x_diff = lidar_x - offset[5][coat][2] + 50;//フェンス
                    if(tmp_goal_x - now_x > 30 && fabs(now_x - point[3][coat][0]) < 400 && fabs(rsx_diff) < 300 && rs_data_x != 0){
                        if(rs_data){
                            goal_x = tmp_goal_x;
                            fprintf(fp,"干し補正X RS_X:%d,%d\tG(%d,%d),N(%d,%d,%f)\n",(int)rs_data_x,(int)rsx_diff,(int)goal_x,(int)goal_y,(int)now_x,(int)now_y,now_yaw);
                        }
                    }else if(lidar_x_diff > 30 && fabs(now_x - point[3][coat][0]) < 300 && fabs(lidar_x_diff) < 300 && fabs(now_yaw) < 2){
                        goal_x = now_x + lidar_x_diff;
                        fprintf(fp,"干し補正X lidar(%d,%d),%d\tG(%d,%d),N(%d,%d,%f)\n",lidar_x,lidar_y,lidar_x_diff,(int)goal_x,(int)goal_y,(int)now_x,(int)now_y,now_yaw);
                    }else if(fabs(now_x - goal_x) < 100){
                        goal_x = now_x + 70;
                        fprintf(fp,"干しリミットまで RS_X:%d,%d\tG(%d,%d),N(%d,%d,%f)\tlidar(%d,%d)\n",(int)rs_data_x,(int)rsx_diff,(int)goal_x,(int)goal_y,(int)now_x,(int)now_y,now_yaw,lidar_x,lidar_y);
                    }else{
                        goal_x = point[3][coat][0];
                        fprintf(fp,"干し補正X 無視lidar(%d,%d),%d\tG(%d,%d),N(%d,%d,%f)\n",lidar_x,lidar_y,lidar_x_diff,(int)goal_x,(int)goal_y,(int)now_x,(int)now_y,now_yaw);
                    }
                }
...
void operate(const std_msgs::Float32MultiArray &data){
    ...
    switch (cmd){
        case -6://リセット
            ...
            break;
        case -5://スタート
            if(spreaded != 1 || mechanism_move != 0){//準備動作
                if(spreaded == 0){//下がってる時は準備
                    sendMechanism(10+fight);
                }else{//上がってる時は下げるだけ
                    sendMechanism(5);
                }
                status = 0;
                next_move = 0;
            }else if(stopped){//非常停止の後の復帰
            ...
            }else if((towel[0] || towel[1] || seats) && status == 0){//フラグなしの時は動かさない
                status = 1;
            }
            ready = false;
            break;
...
inline void autoMove(){
    ...
    if(use_v_max != ((fast_mode && spreaded != 2) ? FAST_V : VMAX[spreaded])){
        use_v_max = ((fast_mode && spreaded != 2) ? FAST_V : VMAX[spreaded]);
    ...
    if((fabs(diff_x) <  (5 + about_move * 5) && fabs(diff_y) < (5 + about_move * 5) && fabs(diff_yaw) < 1
